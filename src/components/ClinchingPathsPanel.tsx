...
export function ClinchingPathsPanel({
  ...
  return (
    <section className="rounded-lg border border-amber-200 bg-amber-50/70 p-5 shadow-xs dark:border-amber-900/60 dark:bg-amber-950/20">
      ...
      {paths.length === 0 ? (
        <div className="rounded-lg border border-dashed border-amber-300 bg-white/70 p-6 text-center text-sm font-bold text-slate-500 dark:border-amber-900 dark:bg-slate-900/50 dark:text-slate-400">
          No paths yet.
        </div>
      ) : (
          ...
          {paths.map((path) => (
            ...
            >
              <div className="mb-3 flex items-start justify-between gap-3">
                ...
                <div className="shrink-0 rounded-full bg-amber-100 px-2.5 py-1 text-xs font-semibold text-amber-800 dark:bg-amber-900/50 dark:text-amber-200">
              ...
              <ul className="space-y-2">
                {path.notes.map((note) => (
                  <li
                    key={note}
                    className="rounded-lg bg-amber-50 px-3 py-2 text-sm font-bold leading-6 text-slate-700 dark:bg-slate-800 dark:text-slate-300"
                  >
                    {note}
                  </li>
                ))}
