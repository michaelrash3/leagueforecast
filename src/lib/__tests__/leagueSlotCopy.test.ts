...
const teams: ScoutTeam[] = [
  { id: "S-513", name: "513 FORCE - BOULEY", gcTeams: [] },
  { id: "S-HORN", name: "Cincinnati Hornets" },
  ...
  { id: "S-OTHER", name: "Some Pulled Club" },
];
const row = (
  id: string,
  a: string,
...
const againstTbd = row("gc_513_1", "S-513", "S-TBD", "2026-09-25", 0, 13);
...
describe("a league game a club's schedule filed against nobody", () => {
  ...
  it("is never a pulled club, which is itself and nobody else", () => {
    const againstOther = row("gc_513_4", "S-513", "S-OTHER", "2026-09-25", 0, 13);
  ...
  it("is a game of its own when the score is not the league's", () => {
    const another = row("gc_513_5", "S-513", "S-TBD", "2026-09-25", 2, 9);
    expect(collapse([league, another])).toEqual([league, another]);
  });

  it("is a game of its own on another day", () => {
    const another = row("gc_513_6", "S-513", "S-TBD", "2026-09-26", 0, 13);
    expect(collapse([league, another])).toEqual([league, another]);
  });
  ...
  it("leaves a day alone when two of the club's rows fit one league game", () => {
    const twin = row("gc_513_7", "S-513", "S-TBD", "2026-09-25", 0, 13);
