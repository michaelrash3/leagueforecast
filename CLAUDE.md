...
- Branches are named after Major League Baseball greats, past and present: last name only,
  lowercase, under the usual prefix — `rash/mays`, `rash/clemente`, `rash/ohtani`. Pick a name not
  already used by a branch or a merged pull request. A session whose branch the web harness named
  keeps that generated name rather than being renamed underneath an open pull request.
