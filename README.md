...
| **Power UX** | ⌘K / Ctrl-K command palette (jump to team/view, share, export, theme, replay tour), `?` shortcuts help, `g s/g g/g m/g t` view jumps, `d` theme toggle, dark mode, shareable URL (state encoded in the hash), CSV import/export with formula-injection guards + BOM strip, undo toasts on destructive actions, 4-step onboarding tour (auto on first run + on demand from the palette). |
| **Installable PWA** | `vite-plugin-pwa` with a minimal manifest + SVG icon + auto-update strategy. Installable on Chromium browsers and iOS Safari ("Add to Home Screen"). No elaborate offline experience — the plugin precaches the build output for installability only. |
| **AI narrative (Gemini, optional)** | Paste a Gemini API key in Settings to swap the deterministic team / impact / game-forecast / compare summaries for short Gemini prose. Key + cache live only in this browser (`nkb_gemini_key_v1`, `nkb_gemini_cache_v1`); never bundled into Backup JSON. Falls back to deterministic copy on missing key, auth error, rate limit, or network failure. |
